...
class ModuleTest {
    ...
    void 'supports running on specific ports'() {
        def builder = SpringMvcBuilder.newSpringMvcServer(dispatcherAndRootBeans).onPort(10101)
    ...
    void 'supports running with a specific context path'() {
        def builder = SpringMvcBuilder.newSpringMvcServer(dispatcherAndRootBeans).atContextPath('/testContextPath/path2')
        thisTestServer = builder.server()
    ...
    void 'fails on setting a context path without a leading slash'() {
        ...
        SpringMvcBuilder.newSpringMvcServer(dispatcherAndRootBeans).atContextPath('noLeadingSlash')
    }
    ...
    void 'supports running at a specific servlet path'() {
        def builder = SpringMvcBuilder.newSpringMvcServer(dispatcherAndRootBeans).mappedTo('/testServletPath/path2/*')
        thisTestServer = builder.server()
